...
> giữa api ↔ replica** (recorder/heatmap). *(Chẩn đoán queue/RabbitMQ làm qua log Loki, không tool riêng.)*
...
> 📌 Đây là greenfield: ta dựng lại từ skeleton, tái sử dụng *pattern* của repo tham chiếu
...
| Tool | Input | Mô tả |
| --- | --- | --- |
| `mongo_compare_replica` | `domain`, `entity`(session\|shop\|pageview\|analytic), `id\|key` | So doc giữa **api (nguồn)** ↔ **recorder/heatmap**: báo *missing / stale (lệch updatedAt) / field-diff*. Phát hiện "recorder lệch" |
