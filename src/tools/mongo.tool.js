...
function wrap(name, fn) {
  return async (args) => {
    try {
      ...
      return errorContent(`${name} lỗi: ${err.message}`, 'Kiểm tra domain, collection whitelist và cấu hình DB URI.');
    }
  };
}
...
export function registerMongoTools(server) {
  ...
  server.registerTool(
    'mongo_resolve_shop',
    {
      ...
      description: 'Bước 0 của mọi RCA theo shop. Tra cứu shard từ Proxy, đọc thông tin Shop (api): status, plan, quota, embed_block, started. Access token bị redact.',
  ...
  server.registerTool(
    'mongo_shop_health',
    {
      ...
      description: 'Kiểm tra shop tồn tại ở api, recorder không. So sánh session_count (field) với đếm thực tế.',
      inputSchema: z.object({
    ...
    wrap('mongo_shop_health', async ({ domain }) => {
      ...
      const result = await withCache(key, MONGO_TTL, async () => {
        ...
        const [apiShop, recShop] = await Promise.allSettled([
          getShopModel(api).findOne({ domain }, { domain: 1, status: 1, session_count: 1, plan_code: 1, proxy: 1 }).lean().exec(),
          recorder ? getRecorderShopModel(recorder).findOne({ domain }, { domain: 1, status: 1, session_count: 1 }).lean().exec() : null,
        ]);
        ...
        if (apiDoc?._id) {
          sessionCountApi = await api.collection('sessions').countDocuments({ shop: apiDoc._id }, { maxTimeMS: 5000 });
        }

        return {
          shardNum,
          api: apiDoc
            ? { exists: true, status: apiDoc.status, plan_code: apiDoc.plan_code, session_count_field: apiDoc.session_count, session_count_actual: sessionCountApi }
            : { exists: false },
          recorder: recDoc ? { exists: true, session_count_field: recDoc.session_count } : { exists: false },
          drift: apiDoc && recDoc
  ...
  server.registerTool(
    ...
    {
      title: 'Mongo Find',
      description: 'Đọc documents từ bất kỳ collection nào trong whitelist. domain tự động resolve shard.',
      inputSchema: z.object({
        db: z.enum(['api', 'heatmap', 'recorder']).describe('Database cần query'),
        collection: z.string().describe(`Collection name. Whitelist: ${Object.entries(COLLECTION_WHITELIST).map(([k, v]) => `${k}: [${v.join(',')}]`).join(' | ')}`),
        filter: z.record(z.unknown()).default({}).describe('MongoDB filter object'),
        projection: z.record(z.number()).default({}).describe('Fields để include (1) hoặc exclude (0)'),
    ...
    wrap('mongo_find', async (args) => {
      ...
      return okContent({ count: docs.length, docs }, { label: `${args.db}.${args.collection} (${docs.length})` });
    }),
  );
  ...
  server.registerTool(
    'mongo_aggregate',
    {
      ...
      description: 'Chạy aggregation pipeline đọc-only. Chặn $out, $merge, $function, $where.',
      inputSchema: z.object({
    ...
    wrap('mongo_aggregate', async (args) => {
      ...
      return okContent({ count: result.length, result }, { label: `Aggregate ${args.db}.${args.collection}` });
    }),
  );
  ...
  server.registerTool(
    ...
    {
      title: 'Mongo Count',
      description: 'Đếm nhanh documents. Không filter = estimatedDocumentCount (nhanh). Có filter = countDocuments (chính xác).',
      inputSchema: z.object({
        db: z.enum(['api', 'heatmap', 'recorder']),
        collection: z.string(),
