...
function wrap(name, fn) {
  return async (args) => {
    ...
    catch (err) { return errorContent(`${name}: ${err.message}`); }
  };
}

export function registerMongoIntegrityTools(server) {
  server.registerTool('mongo_compare_replica', {
    title: 'Mongo Compare Replica',
    description: 'So sánh document giữa api (nguồn) ↔ recorder. Phát hiện MISSING (recorder không có) hoặc STALE (lệch fields). Khi phát hiện drift → chạy loki_queue_health để xác nhận nguyên nhân.',
  ...
  server.registerTool('mongo_missing_report', {
    title: 'Mongo Missing Report',
    description: 'Đọc sessionMissing + analytic_missing từ Recorder — những gì hệ thống tự đánh dấu là thiếu. Dấu hiệu trực tiếp của consumer/queue lỗi.',
    inputSchema: z.object({
      domain: z.string(),
  ...
  server.registerTool('mongo_replica_lag', {
    title: 'Mongo Replica Lag',
    description: 'Ước lượng độ trễ replica: so last_active mới nhất ở api vs recorder. Lag > 5min = cờ đỏ.',
    inputSchema: z.object({
      domain: z.string(),
    }),
  }, wrap('mongo_replica_lag', async ({ domain }) => {
    const result = await replicaCompareService.replicaLag(domain);
    return okContent(result, { label: `Replica lag: ${domain} — ${result.lag_human ?? 'unknown'}` });
  }));
}
