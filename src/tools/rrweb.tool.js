...
function wrap(name, fn) {
    return async (args) => {
        try {
            ...
            return errorContent(
                `${name}: ${err.message}`,
                'Kiểm tra domain, sessionId/pageViewId và đảm bảo Playwright/Chromium đã được cài (npx playwright install chromium).',
            );
        }
    };
}
...
export function registerRrwebTools(server) {
    server.registerTool(
        'rrweb_list',
        {
            title: 'rrweb List',
            description:
                'Liệt kê các PageView có rrweb Events khả dụng để replay, theo session hoặc khoảng thời gian.',
        ...
        wrap('rrweb_list', async ({ domain, sessionId, start, end, limit }) => {
            ...
            const pageviews = await PageView.find(pvFilter, {
                href: 1,
                session: 1,
                page: 1,
                start_time: 1,
            ...
            return okContent(
                { count: list.length, pageviews: list },
                { label: `rrweb list: ${list.length} pageviews with events` },
            );
        }),
    );
    ...
    server.registerTool(
        'rrweb_render',
        {
            title: 'rrweb Render',
            description:
                'Render rrweb session replay từ MongoDB Events, chụp screenshot tại mốc thời gian. Trả về ảnh PNG.',
            inputSchema: z.object({
                domain: z.string(),
                ...
                atMs: z
                    .number()
                    .optional()
                    .describe('Mốc thời gian ms kể từ đầu replay (null = cuối)'),
    ...
    server.registerTool(
        'screenshot_url',
        {
            ...
            description:
                'Chụp screenshot website thật (storefront, page cụ thể) dùng Playwright. Dùng để so sánh với rrweb replay.',
            inputSchema: z.object({
    ...
    server.registerTool(
        'rrweb_diagnose',
        {
            ...
            description:
                'So sánh rrweb replay snapshot ↔ live website: render cả hai, tính pixelmatch diff. Để AI suy luận lỗi hiển thị/JS. Trả về 3 ảnh: replay, live, diff.',
            inputSchema: z.object({
                domain: z.string(),
