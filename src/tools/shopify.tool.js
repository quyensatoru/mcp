...
function wrap(name, fn) {
    return async (args) => {
        try {
            ...
            return errorContent(
                `${name}: ${err.message}`,
                'Kiểm tra SHOPIFY_API_VERSION và Shopify Admin API token trong DB.',
            );
        }
    };
}
...
export function registerShopifyTools(server) {
    server.registerTool(
        'shopify_graphql',
        {
            ...
            description:
                'Chạy GraphQL query lên Shopify Admin API (token lấy từ DB). Dùng để kiểm tra theme, app embed, script tags, webhooks.',
        ...
        wrap('shopify_graphql', async ({ domain, query, variables }) => {
            ...
            const result = await withCache(key, SHOPIFY_TTL, () =>
                shopifyService.graphql(domain, query, variables),
            );
    ...
    server.registerTool(
        'shopify_rest',
        {
            title: 'Shopify REST',
            description:
                'Gọi Shopify Admin REST API (GET only). Dùng khi cần data không có trong GraphQL.',
            inputSchema: z.object({
                domain: z.string(),
        ...
        wrap('shopify_rest', async ({ domain, path }) => {
            ...
            const result = await withCache(key, SHOPIFY_TTL, () =>
                shopifyService.rest(domain, path, 'GET'),
            );
    ...
    server.registerTool(
        'shopify_check_embed',
        {
            ...
            description:
                'Kiểm tra app embed / script tag Mida có active trong Shopify theme không. Là một trong những check đầu tiên khi "session không ghi".',
            inputSchema: z.object({
                domain: z.string(),
            }),
        },
        wrap('shopify_check_embed', async ({ domain }) => {
            ...
            const result = await withCache(key, SHOPIFY_TTL, () =>
                shopifyService.checkEmbed(domain),
            );
