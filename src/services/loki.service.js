...
function summarizeLines(lines) {
  ...
  for (const { line, labels } of lines) {
    const lvl = labels?.level ?? (line.includes('error') ? 'error' : line.includes('warn') ? 'warn' : 'info');
...
export const lokiService = {
  ...
  async queueHealth({ channel, service, start, end, limit = 200 }) {
    const keywords = ['nack', 'reject', 'channel error', 'connection drop', 'no consumer', 'backlog', 'slow'];
