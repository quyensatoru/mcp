...
function tryId(s) { try { return new ObjectId(s); } catch { return s; } }

function fieldDiff(apiDoc, repDoc, fields) {
  const diffs = [];
  for (const f of fields) {
    const a = apiDoc?.[f], r = repDoc?.[f];
...
export const replicaCompareService = {
  async compareSession(domain, sessionId) {
    ...
    const recDoc = RecSession ? await RecSession.findById(tryId(sessionId)).lean().exec() : null;

    if (!apiDoc) return { status: 'NOT_IN_API', sessionId };
    if (!recDoc) return { status: 'MISSING_IN_RECORDER', sessionId, apiExists: true, hint: 'Recorder consumer có thể lỗi. Chạy loki_queue_health với channel=recorder-backup.' };

    const compareFields = ['key', 'status', 'duration', 'last_active', 'page_per_session', 'click_count'];
    const diffs = fieldDiff(apiDoc, recDoc, compareFields);
    const lagMs = apiDoc.updatedAt && recDoc.updatedAt
    ...
    return {
      ...
      lagMs,
      diffs,
      hint: diffs.length ? 'Recorder có data nhưng lệch — consumer xử lý chậm hoặc partial failure.' : null,
    };
  },
  ...
  async compareShop(domain) {
    ...
    const apiDoc = await Shop.findOne({ domain }, { domain: 1, status: 1, plan_code: 1, session_count: 1, embed_block: 1 }).lean().exec();
    const recDoc = RecShop ? await RecShop.findOne({ domain }, { domain: 1, status: 1, plan_code: 1, session_count: 1 }).lean().exec() : null;
  ...
  async missingReport(domain, dateFrom, dateTo) {
    ...
    if (dateTo) baseFilter.createdAt = { ...baseFilter.createdAt, $lte: new Date(dateTo + 'T23:59:59Z') };

    const [sessionMissing, analyticMissing] = await Promise.all([
      SessionMissing.find(baseFilter, { key: 1, createdAt: 1, device: 1, browser: 1, location: 1 }).sort({ createdAt: -1 }).limit(100).lean().exec(),
      AnalyticMissing.find({ shop: shop._id, ...(dateFrom ? { date: { $gte: new Date(dateFrom) } } : {}) }, { date: 1, count_session: 1, createdAt: 1 }).sort({ date: -1 }).limit(30).lean().exec(),
    ]);
    ...
    return {
      ...
      diagnosis: sessionMissing.length || analyticMissing.length
  ...
  async replicaLag(domain) {
    ...
    const [latestApi, latestRec] = await Promise.all([
      Session.findOne(shopFilter, { last_active: 1, updatedAt: 1 }).sort({ last_active: -1 }).lean().exec(),
      RecSession.findOne(shopFilter, { last_active: 1, updatedAt: 1 }).sort({ last_active: -1 }).lean().exec(),
    ]);
    ...
    return {
      ...
      status: lagMs == null ? 'UNKNOWN' : lagMs > 300000 ? '⚠️ LAG > 5min' : lagMs > 60000 ? '⚠️ LAG > 1min' : '✅ OK',
    };
  },
};
