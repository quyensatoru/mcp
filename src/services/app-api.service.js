...
function assertPath(path) {
  if (!ALLOWED_PREFIXES.some(p => path.startsWith(p))) {
    throw new Error(`Path "${path}" không có trong whitelist. Allowed: ${ALLOWED_PREFIXES.join(', ')}`);
  }
}
...
export const appApiService = {
  async call(path, method = 'GET', params) {
    ...
    if (!resp.ok) throw new Error(`App API ${method} ${path} → ${resp.status}: ${await resp.text()}`);
    return resp.json();
  },
