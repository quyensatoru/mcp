...
export const shopifyService = {
  ...
  async rest(domain, path, method = 'GET', body) {
    ...
    if (!resp.ok) throw new Error(`Shopify REST ${method} ${path} → ${resp.status}: ${await resp.text()}`);
    return resp.json();
  },

  async checkEmbed(domain) {
    // Kiểm tra app embed + script tag Mida trong theme hiện tại
    const themeData = await this.graphql(domain, `
      query {
    ...
    const midaScript = scriptTags.find(s => s.src?.includes('mida') || s.src?.includes('getmida'));
    return {
      mainTheme,
      scriptTags,
      midaScriptFound: !!midaScript,
      midaScript: midaScript ?? null,
      diagnosis: midaScript ? '✅ Mida script tag tồn tại' : '⚠️ Không tìm thấy Mida script tag — có thể app embed bị tắt hoặc theme không nhúng snippet',
    };
  },
};
