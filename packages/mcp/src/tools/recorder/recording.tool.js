...
function formatMissing(data, shop, domain) {
    ...
    const lines = [
        ...
        ...sessionMissing
            ...
            .map(
                (s, i) =>
                    `  ${i + 1}. ${s.device ?? '?'}/${s.browser ?? '?'} · ${s.location ?? '?'} · ${s.createdAt ? new Date(s.createdAt).toISOString() : '—'}`,
            ),
        '',
        ...
        ...analyticMissing
            ...
            .map(
                (a) =>
                    `  ${a.date ? new Date(a.date).toISOString().slice(0, 10) : '—'}: ${a.count_session ?? 0} sessions`,
            ),
    ];
    return lines.join('\n');
}
...
export function registerRecordingTools(server) {
    server.registerTool(
        ...
        wrap('recording_integrity', async ({ domain, sessionId }) => {
            const data = await withCache(
                cacheKey('recording_integrity', { domain, sessionId }),
                TTL,
                async () => {
    ...
    server.registerTool(
        ...
        wrap('recording_missing', async ({ domain, dateFrom, dateTo }) => {
            ...
            const data = await withCache(
                cacheKey('recording_missing', { domain, dateFrom, dateTo }),
                TTL,
