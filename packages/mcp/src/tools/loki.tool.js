...
function wrap(fn) {
    return async (args) => {
        try {
            ...
            return errorContent(
                `Loki error: ${err.message}`,
                'Kiểm tra LOKI_URL, LOKI_TOKEN/USER/PASS và kết nối mạng.',
            );
        }
    };
}
...
export function registerLokiTools(server) {
    server.registerTool(
        'loki_query',
        {
            ...
            description:
                'Chạy truy vấn LogQL tuỳ chỉnh trên Loki. Dùng khi cần query nâng cao. Trả log lines + summary.',
    ...
    server.registerTool(
        'loki_search_errors',
        {
            ...
            description:
                'Tìm log lỗi (error/warn) theo app/service và tuỳ chọn lọc theo shopDomain. Điểm khởi đầu tốt cho RCA.',
            inputSchema: z.object({
                ...
                shop: z
                    .string()
                    .optional()
                    .describe('Lọc thêm theo shop domain trong nội dung log'),
    ...
    server.registerTool(
        'loki_trace',
        {
            title: 'Loki Trace',
            description:
                'Gom toàn bộ log theo traceId/requestId/correlationId. Xương sống của RCA — trace 1 request qua các service.',
            inputSchema: z.object({
        ...
        wrap(async (args) => {
            ...
            return okContent(result, {
                label: `Trace ${args.traceId} (${result.lines.length} lines)`,
            });
        }),
    );
    ...
    server.registerTool(
        'loki_queue_health',
        {
            ...
            description:
                'Tìm log lỗi RabbitMQ/consumer trong Loki (nack, reject, channel error, no consumer, backlog). Dùng khi phát hiện replica lệch từ mongo_compare_replica.',
            inputSchema: z.object({
                channel: z
                    .string()
                    .optional()
                    .describe('Tên channel/queue, vd: "recorder-backup", "heatmap"'),
            ...
            return okContent(
                {
                    ...result,
                    diagnosis: hasErrors
                        ? '⚠️ Phát hiện lỗi queue/consumer'
                        : '✅ Không có lỗi queue trong khoảng thời gian này',
                },
    ...
    server.registerTool(
        'loki_labels',
        {
            title: 'Loki Labels',
            description:
                'Liệt kê Loki labels hoặc giá trị của 1 label. Dùng để khám phá app/service names trước khi query.',
            inputSchema: z.object({
                name: z
                    .string()
                    .optional()
                    .describe(
                        'Tên label cụ thể để lấy values, vd: "app". Để trống = liệt kê tất cả labels.',
                    ),
                ...timeRangeSchema.shape,
            }),
        },
