...
function wrap(name, fn) {
    return async (args) => {
        try {
            ...
            return errorContent(
                `${name}: ${err.message}`,
                'Kiểm tra domain, sessionId/pageViewId đúng định dạng ObjectId hoặc session key.',
            );
        }
    };
}
...
export function registerMongoPipelineTools(server) {
    ...
    server.registerTool(
        'mongo_session_trace',
        {
            ...
            description:
                'Đi dọc pipeline Session→PageView→Event/Behavior, đếm ở từng tầng, cắm cờ chỗ đứt. Ví dụ: có Session nhưng 0 PageView = recording hỏng ở tầng ingest. Đây là tool chẩn đoán "không ghi" cốt lõi.',
        ...
        wrap('mongo_session_trace', async ({ domain, sessionId, sessionKey }) => {
            ...
            const result = await withCache(key, MONGO_TTL, async () => {
                ...
                if (!session)
                    return {
                        found: false,
                        message: `Session không tồn tại trong api (shard=${domain})`,
                    };
                ...
                const flags = [];
                if (!pageviews.length)
                    flags.push('⚠️ Session tồn tại nhưng 0 PageView — ingest/queue có thể lỗi');
                else if (eventCount === 0)
                    flags.push(
                        '⚠️ Có PageView nhưng 0 Event (rrweb) — recording script không gửi data hoặc consumer drop',
                    );
                ...
                return {
                    session: {
                        ...
                        page_per_session: session.page_per_session,
                    },
                    counts: {
    ...
    server.registerTool(
        'mongo_get_session',
        {
            ...
            description:
                'Lấy full session doc + visitor liên kết. Dùng sau mongo_session_trace để xem chi tiết.',
            inputSchema: z.object({
                domain: z.string(),
        ...
        wrap('mongo_get_session', async ({ domain, sessionId, sessionKey }) => {
            ...
            const visitor = session.visitor
                ? await Visitor.findById(session.visitor).lean().exec()
                : null;
    ...
    server.registerTool(
        'mongo_get_pageviews',
        {
            ...
            description:
                'Danh sách PageView của một session (href, page_type, theme_template, timestamps).',
            inputSchema: z.object({
                domain: z.string(),
        ...
        wrap('mongo_get_pageviews', async ({ domain, sessionId }) => {
            ...
            const pageviews = await PageView.find(
                ...
                .exec();
            return okContent(
                { count: pageviews.length, pageviews },
                { label: `PageViews of session ${sessionId}` },
            );
        }),
    );
    ...
    server.registerTool(
        'mongo_get_behaviors',
        {
            ...
            description:
                'Behavior events (cart, funnel, ux issue) theo session hoặc pageView. Dùng để chẩn đoán funnel/conversion issues.',
            inputSchema: z.object({
                domain: z.string(),
        ...
        wrap('mongo_get_behaviors', async ({ domain, sessionId, pageViewId, limit }) => {
            ...
            const behaviors = await Behavior.find(filter)
                ...
                .exec();
            return okContent(
                { count: behaviors.length, behaviors },
    ...
    server.registerTool(
        'mongo_get_analytic',
        {
            ...
            description:
                'Lấy Analytic theo ngày (visitor/session counts, order_funnel, bounce/conversion_rate). Dùng để đối chiếu "số liệu có cộng dồn đúng không".',
            inputSchema: z.object({
                domain: z.string(),
