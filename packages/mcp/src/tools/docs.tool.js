...
export function registerDocsTool(server) {
    server.registerTool(
        'docs_search',
        {
            title: 'Docs Search',
            description:
                'Tìm trong tài liệu Mida. Dùng trước khi điều tra để hiểu hành vi kỳ vọng của hệ thống.',
        ...
        async ({ query, topK }) => {
            try {
                ...
                if (!results.length) {
                    return okContent({
                        results: [],
                        note: 'Không tìm thấy. Thử từ khoá khác hoặc đọc mida-doc://index.',
                    });
                }
                return okContent(
                    results.map((r) => ({
